...
VALID_SETTINGS = {"pretrained", "random"}
...
def randomize_clip_weights(clipmodel, seed):
...
def build_model_and_explainer(setting, clip_model_name, device, random_seed):
    clipmodel, preprocess = clip.load(clip_model_name, device=device)
    if setting == "random":
        randomize_clip_weights(clipmodel, random_seed)
    clipmodel.eval()
    explainer = CLIPExplainRunner(clipmodel=clipmodel, preprocess=preprocess, device=device)
    return clipmodel, preprocess, explainer
...
def evaluate_setting(
    ...
    clipmodel, preprocess, explainer = build_model_and_explainer(
        setting=setting,
    ...
    if not valid_methods:
        raise RuntimeError(f"No valid curves were produced for setting: {setting}")

    del_mean = {m: (del_sum[m] / counts[m]) for m in valid_methods}
    ins_mean = {m: (ins_sum[m] / counts[m]) for m in valid_methods}
...
def main():
    ...
    parser.add_argument(
        ...
        help="Comma-separated model settings: pretrained,random",
    )
    parser.add_argument("--random-seed", type=int, default=0, help="Seed used to randomize CLIP weights")
    parser.add_argument(
        "--methods",
    ...
    settings = [s.strip().lower() for s in args.settings.split(",") if s.strip()]
    invalid_settings = [s for s in settings if s not in VALID_SETTINGS]
    if invalid_settings:
        raise ValueError(f"Unsupported settings: {invalid_settings}. Valid: {sorted(VALID_SETTINGS)}")
    if not settings:
    ...
    fig, axes = plt.subplots(len(results), 2, figsize=(14, 5 * len(results)), dpi=120, squeeze=False)
    summary_lines = [
    ...
    for row_idx, result in enumerate(results):
        ...
        x_del = result["x_del"]
        x_ins = result["x_ins"]
        valid_methods = result["valid_methods"]
        counts = result["counts"]

        for hm_type in valid_methods:
            del_mean = result["del_mean"][hm_type]
            ins_mean = result["ins_mean"][hm_type]
            axes[row_idx, 0].plot(x_del, del_mean, marker="o", label=hm_type)
            axes[row_idx, 1].plot(x_ins, ins_mean, marker="o", label=hm_type)

            del_auc = float(np.trapz(del_mean, x_del))
            ins_auc = float(np.trapz(ins_mean, x_ins))
            summary_lines.append(
                f"{setting}\t{hm_type}\tcount={counts[hm_type]}\tdel_auc={del_auc:.6f}\tins_auc={ins_auc:.6f}"
            )

        axes[row_idx, 0].set_title(f"[{setting}] Deletion Curve (mean on ImageNet val)")
        axes[row_idx, 0].set_xlabel("Removed Pixel Ratio")
        axes[row_idx, 0].set_ylabel("P(eval class)")
        axes[row_idx, 0].grid(True, alpha=0.3)

        axes[row_idx, 1].set_title(f"[{setting}] Insertion Curve (mean on ImageNet val)")
        axes[row_idx, 1].set_xlabel("Inserted Pixel Ratio")
        axes[row_idx, 1].set_ylabel("P(eval class)")
